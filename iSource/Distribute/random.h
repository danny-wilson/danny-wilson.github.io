...
public:
	...
	{
		if(a<=0.0 || b<=0.0) rerror("Error in beta: a and b parameters must be >0");
		double gam1,gam2;
	...
	/* Returns the random variates in the Vector MN */
	void multivariate_normal(Vector<double> &mu, Matrix<double> &Sigma, Vector<double> &MN, Matrix<double> &temp, Vector<double> &z) {
			...
			for(j=i;j<n;j++) {
				...
				if(i==j) {
					if(sum <= 0.0) {/* Sigma, with rounding errors, is not positive definite */
						...
						rerror("multivariate_normal(): Cholesky decomposition failed, not positive definite");
					}
