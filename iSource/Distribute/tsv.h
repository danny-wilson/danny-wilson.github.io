...
//			if(!infile.good()) error("Problem reading file - is buffer too small?");
		...
		if(coutput) cout << "(" << nfields << " fields in total)" << endl;
		...
		while(!infile.eof())
		{
			...
			if(dump[0]!='\0')/*check the line isn't blank using the end-of-string character*/
				++nrows;
			++ntries;
			if(coutput && ntries%1000==0) cout << "\r" << ntries << " attempts, " << nrows << " rows so far";
		}	cout << endl;
		if(coutput) cout << "Found " << nrows << " rows of data" << endl;
